...
parser_error_t _read_cro_redeem_address(parser_context_t *c, cro_redeem_address_t *v) {
    CHECK_INPUT()
...
parser_error_t _read_cro_extended_address(parser_context_t *c, cro_extended_address_t *v) {
    CHECK_INPUT()
    uint8_t address_type;
    CHECK_PARSER_ERR(_readUInt8(c, &address_type))
    if (address_type != 0) {
...
parser_error_t _read_cro_option_timespec(parser_context_t *c, cro_option_timespec_t *v) {
    CHECK_PARSER_ERR(_readUInt8(c, &v->has_value))
    if (v->has_value) {
...
parser_error_t
_read_cro_option_validator_security_contact(parser_context_t *c, cro_option_validator_security_contact_t *v) {
    CHECK_PARSER_ERR(_readUInt8(c, &v->has_value))
    if (v->has_value) {
...
parser_error_t _read_cro_secp256k1_pubkey(parser_context_t *c, cro_secp256k1_pubkey_t *v) {
    GEN_DEF_READARRAY(CRO_SECP256K1_PUBKEY_SIZE)
}
...
parser_error_t _read_cro_council_node(parser_context_t *c, cro_council_node_t *v) {
    ...
    CHECK_PARSER_ERR(_read_cro_option_validator_security_contact(c, &v->security_contact));
...
parser_error_t _read_cro_tx_out(parser_context_t *c, cro_tx_out_t *v) {
    CHECK_INPUT()
    CHECK_PARSER_ERR(_read_cro_extended_address(c, &v->address));
    CHECK_PARSER_ERR(_read_cro_coin(c, &v->value));
    CHECK_PARSER_ERR(_read_cro_option_timespec(c, &v->valid_from));
    return parser_ok;
}
...
parser_error_t _read_cro_tx_access(parser_context_t *c, cro_tx_access_t *v) {
    CHECK_INPUT()
    CHECK_PARSER_ERR(_readUInt8(c, &v->value));
    if (v->value != 0) {
        return parser_unexpected_value;
    }
    return parser_ok;
...
parser_error_t _read_cro_access_policy(parser_context_t *c, cro_access_policy_t *v) {
    CHECK_INPUT()
    CHECK_PARSER_ERR(_read_cro_secp256k1_pubkey(c, &v->key))
    CHECK_PARSER_ERR(_read_cro_tx_access(c, &v->access));
    return parser_ok;
}

parser_error_t _read_cro_vector_access_policy(parser_context_t *c, cro_vector_access_policy_t *v) {
    GEN_DEF_READVECTOR(access_policy);
}

parser_error_t _read_cro_tx_attributes(parser_context_t *c, cro_tx_attributes_t *v) {
    CHECK_INPUT()
    uint8_t dummy;
    CHECK_PARSER_ERR(_readUInt8(c, &dummy));
    if (dummy != 0) {
        return parser_unexpected_value;
    }
    CHECK_PARSER_ERR(_readUInt8(c, &v->chain_id));
    CHECK_PARSER_ERR(_read_cro_vector_access_policy(c, &v->allowed_view));
    CHECK_PARSER_ERR(_readUInt64(c, &v->app_version));
    return parser_ok;
}
...
parser_error_t _read_withdraw_unbounded_tx(parser_context_t *c, cro_withdraw_unbonded_tx_t *v) {
    CHECK_INPUT()
    CHECK_PARSER_ERR(_read_cro_nonce(c, &v->nonce));
    CHECK_PARSER_ERR(_read_cro_vector_tx_out(c, &v->outputs));
...
parser_error_t _read(parser_context_t *c, parser_tx_t *v) {
    ...
    switch (v->txAuxEnumType) {
        case CRO_TX_AUX_ENUM_ENCLAVE_TX: {
            ...
            switch (v->txType) {
                case CRO_TX_AUX_ENCLAVE_WITHDRAW_UNBOUNDED_STAKE: {
                    CHECK_PARSER_ERR(_read_withdraw_unbounded_tx(c, &v->cro_withdraw_unbounded_tx))
                    return parser_ok;
                }
                default:
        ...
        case CRO_TX_AUX_ENUM_ENCLAVE_TX:
            switch (v->txType) {
                case CRO_TX_AUX_ENCLAVE_WITHDRAW_UNBOUNDED_STAKE: {
                    ...
                    const uint16_t addressItemCount = v->cro_withdraw_unbounded_tx.outputs._len * 3;
                    const uint16_t accessPolicyItemCount = v->cro_withdraw_unbounded_tx.attributes.allowed_view._len * 2;
                    const uint16_t attributesItemCount = 2 + accessPolicyItemCount;
                    const uint16_t itemCount = fixStartCount + addressItemCount + attributesItemCount;
                    return itemCount;
                }
            }
            return 0;
